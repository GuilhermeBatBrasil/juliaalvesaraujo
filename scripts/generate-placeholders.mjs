 ...
 *
 * Os arquivos gerados servem apenas para visualizar o layout e devem ser
 * substituídos pelas fotos reais, mantendo os mesmos nomes.
 */
...
for (const { name, width, height, color, label } of images) {
  ...
  await writeFile(path.join(OUTPUT_DIR, `${name}.jpg`), buffer);
